...
def spotbugs_report(cmd, folder_path, log_file, lock):
...
def spotbugs_all_projects(work_folder, file_type='html'):
            ...
            if root.count(os.sep) == 5:
                for folder in natsorted(dirs):
                    if folder[-5:] == 'buggy':
                        ...
                        output_path = f'{defects4j_dir}/spotbugs-{file_type}-reports/{pid}'
                        os.makedirs(output_path, exist_ok=True)
                        output_file = os.path.join(f'{output_path}', f'{folder}-spotbugs_report.{file_type}')
                        os.system(f'touch {output_file}')
                        if os.path.exists(exclude_filter_path):
                            cmd = f'spotbugs -{file_type} -exclude {exclude_filter_path} -output {output_file} {target_path}'
                        else:
                            cmd = f'spotbugs -{file_type} -output {output_file} {target_path}'
                        log_dir = os.path.join(defects4j_dir, f'logs/{pid}')
                        os.makedirs(log_dir, exist_ok=True)
                        log_file_path = os.path.join(log_dir, f'{folder}-spotbugs.log')
                        lock = threading.Lock()
                        executor.submit(spotbugs_report, cmd, work_folder, log_file_path, lock)

       
if __name__ == '__main__':
   spotbugs_all_projects(f'{defects4j_dir}/tmp')
