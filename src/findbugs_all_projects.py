...
def findbugs_all_projects(work_folder):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        for root, dirs, files in os.walk(work_folder):
            # Traverse all fixed versions, 835 in total 
            if root.count(os.sep) == 5:
                for folder in natsorted(dirs):
                    if folder[-5:] == 'fixed':
                        ...
                        output_path = f'{defects4j_dir}/spotbugs-fixed-html-reports/{pid}'
                        os.makedirs(output_path, exist_ok=True)
                        output_file = os.path.join(f'{output_path}', f'{folder}-spotbugs_report.html')
                        os.system(f'touch {output_file}')
                        if os.path.exists(exclude_filter_path):
                            cmd = f'spotbugs -html -output {output_file} -exclude {exclude_filter_path} {target_path}'
                        else:
                            cmd = f'spotbugs -html -output {output_file} {target_path}'
